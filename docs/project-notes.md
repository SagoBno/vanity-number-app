...
The vanity number algorithm is deterministic and local. It focuses on the last seven digits of the caller number, checks a curated word list first, creates hybrid candidates when an embedded word can be preserved, then generates a capped candidate set. Candidates score higher when they match known words, contain partial words, use balanced vowels, avoid long repeated or consonant-heavy runs, and convert more of the seven-digit segment. I avoided external word APIs so the Lambda remains fast, predictable, and inexpensive.
...
Amazon Connect could not be fully tested immediately because newly created AWS accounts can require time before Connect is available. To keep the project reviewable before that access was available, the repository included contact flow content, an optional Connect CloudFormation template, and manual setup instructions. After Connect became available, the flow was deployed and validated with a claimed phone number.

The Amazon Connect CloudFormation path exposed several useful production lessons. The deployment role needed `connect:TagResource`, `connect:UntagResource`, and `connect:ListLambdaFunctions` in addition to the obvious create/update/delete actions. The first flow content also used a couple of invalid flow-language parameters, so the final template now uses `FlowLoggingBehavior` for logging and omits unsupported `TextType` values from `MessageParticipant`.
...
The vanity algorithm uses a curated local vocabulary rather than a large dictionary or language model. That keeps the Lambda small and deterministic, but some phone numbers will still produce pronounceable-looking letter combinations rather than clean words.

## With More Time

I would expand the vanity ranking with a larger curated dictionary, support for more partial-word layouts, and language-aware scoring so non-word combinations are less likely to appear in the top three.
