...
The dashboard API returns only masked caller numbers, but the DynamoDB table still stores the full caller number. In a stricter production environment I would store only the last four digits plus a keyed hash/token unless the full number had a clear business need.
...
I would complete a live Amazon Connect phone-number test once the AWS account can use Connect, then add the real phone number and exact test flow notes to the README.
...
I would harden PII storage by replacing full caller-number persistence with a tokenized or hashed representation plus minimal display fields.
